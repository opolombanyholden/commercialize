...
class QuoteController extends Controller
{
    ...
    public function generate(Request $request)
    {
        ...
        try {
            ...
            // Nom du fichier
            $filename = 'Devis_' . $quoteData['quote']['number'] . '_' . now()->format('Ymd') . '.pdf';
    ...
    /**
     * Vérifie le mot de passe et télécharge le PDF
     */
     ...
     */
    private function sendPassword($pendingQuote, $method) {
        $password = $pendingQuote['pdf_password'];
        $quoteNumber = $pendingQuote['quote_number'];
        
        if ($method === 'email' && $pendingQuote['client_email']) {
            $this->sendPasswordByEmail($pendingQuote['client_email'], $password, $quoteNumber);
        } elseif ($method === 'sms' && $pendingQuote['client_phone']) {
            $this->sendPasswordBySms($pendingQuote['client_phone'], $password, $quoteNumber);
        }
    }
